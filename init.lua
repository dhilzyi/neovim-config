...
require("lazy").setup({
	...
	-- {
	{
		...
		dependencies = {
			...
			"leoluz/nvim-dap-go", -- We keep this for the 'test' helper features
		},
		config = function()
			...
			-- Setup Mason to install debuggers
			...
			---------------------------------------------------------
			-- 1. GO CONFIGURATION (Manual & Robust)
			---------------------------------------------------------
			dap.adapters.go = {
				type = "server",
				port = "${port}",
				executable = {
					-- This tells Neovim to use 'dlv' (Delve) which Mason installs
					command = "dlv",
			...
			dap.configurations.go = {
				...
				},
				{
					...
					request = "launch",
					program = "${workspaceFolder}", -- Run the whole folder/package
				},
			}

			---------------------------------------------------------
			-- 2. C++ CONFIGURATION (Manual & Robust)
			---------------------------------------------------------
			dap.adapters.codelldb = {
				type = "server",
				port = "${port}",
				executable = {
					-- POINT THIS TO WHERE MASON INSTALLS CODELLDB
					-- Windows path usually looks like this:
...
local function switch_to_tab()
	...
	-- 🟢 THE FIX: Only start terminal if buffer has NO NAME and is empty.
	-- If you opened "notes.txt", it has a name, so we skip this!
